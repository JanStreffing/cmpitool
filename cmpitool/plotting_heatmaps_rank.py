def plotting_heatmaps_rank(models, regions, seasons, obs, error_fraction, cmpi, out_path, verbose):
    '''
    AUTHORS:
    ...
    for model in tqdm(models):
            ...
            for depth in var.depths:
                for region in regions:
                    for seas in seasons:
                        try:
                            if len(var.depths) == 1:
                                print('error_fraction ',error_fraction[var.name,depth,seas,model.name,region.name])
                                reorganized_error_fraction[var.name+' '+region.name,depth+' '+seas]=error_fraction[var.name,depth,seas,model.name,region.name]
                            else:
                                reorganized_error_fraction[var.name+' '+region.name,depth+' '+seas]=error_fraction[var.name,depth,seas,model.name,region.name]
                            r+=1
                        except:
        ...
        ax = sns.heatmap(collect_frac_dataframe, vmin=1., vmax=30,center=15,annot=True,fmt='.2f',cmap="PiYG_r",cbar=False,linewidths=1)
        plt.xticks(rotation=90,fontsize=14)
        plt.yticks(rotation=0, ha='right',fontsize=14)
        plt.title(model.name+' CMPI: '+str(round(cmpi[model.name],3)), fontsize=18)
        
        plt.savefig(out_path+'plot/'+model.name+'_rank.png',dpi=300,bbox_inches='tight')

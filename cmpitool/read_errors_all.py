def read_errors_all(obs, eval_models, regions, seasons, out_path, eval_path, n_implemented_var, verbose):
    '''
    AUTHORS:
    ...
    with warnings.catch_warnings():
        ...
        #ensmean=np.nanmean(collect,axis=0)
        vals = np.sort(collect, axis=0)
        print('collect ',collect[0,0,0,0,0])


    print('Placing sums of error into easier to inspect dictionary')

    eval_error_all = OrderedDict()
    ...
    for var in obs:
        ...
        for region in regions:
            ...
            for depth in var.depths:
                m=0
                for seas in seasons:
                    eval_error_all[var.name,region.name,depth,seas]=collect[:,j,k,l,m]
                    m+=1
                l+=1
            k+=1
        j+=1

    return eval_error_all
